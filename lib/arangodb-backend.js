...
var contract = require('../node_modules/acl/lib/contract');
...
ArangoDBBackend.prototype = {
  ...
  */
  get : function(bucket, key, cb){
    ...
    collection.firstExample(searchParams, function(error, response){
        ...
        cb(undefined, _.without(_.keys(doc), "key", "_id", "_key", "_rev"));
    });
  },
  ...
  */
  union : function(bucket, keys, cb){
    ...
    this.db.query(qb.for('doc').in(collName).filter(qb.in('doc.key', qb(keys))).return('doc').toAQL(), function(err, cursor){
  ...
  */
  del : function(transaction, bucket, keys){
    ...
    transaction.push(function(cb){
        // self.db.query(delQuery, {collName: collName, keys: JSON.stringify(keys)}, undefined, function(err, cursor){
        self.db.query(qb.for('doc').in(collName).filter(qb.in('doc.key', qb(keys))).remove('doc').in(collName).toAQL(), function(err, cursor){
            if(err instanceof Error) return cb(err);
            cb(undefined);
        });
    });
